...
Chainbase API [getTokenPriceHistory](/reference/gettokenpricehistory) takes the chain id, the contract address of a certain ERC20 token, start time and end time as parameters, and returns the historical price of that token during that time period.
