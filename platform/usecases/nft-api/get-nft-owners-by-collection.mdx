...
Chainbase API [getNFTOwners](/reference/getnftowners) takes the chain id and NFT contract address as parameters, and returns the holders’ address and the corresponding total. You can also get the total holder number in the `count` column of the returned data.
