...
$MM is our native token, for more detail TBD
...
- For the $MM part, stakers interact with erc20-stake contract to emit a `Stake(bytes operator_pubkey, uint256 amount)` event. The relayer then watches for this event and syncs the information with the Cosmos part.
- We choose the Native Dual Staking model where an oracle ensures fair price conversion of both `$LST` and `$MM` into `$MM-Stake` units.
# **reference**
